...
class Navigation
{
    ...
    protected function makeSidebar()
    {
        $this->navigation->create(Navigable::MENU_SIDEBAR, function (MenuBuilder $sidebar) {
            ...
            $sidebar->dropdown(trans('administrator::module.settings'), function (MenuItem $sub) {
                $sub->route('translations.index', trans('administrator::module.resources.translations'), [], 9, ['id' => 'translations']);
            }, 8, ['id' => 'settings', 'icon' => 'fa fa-folder']);
        });
    }
    ...
    protected function makeTools()
    {
        $this->navigation->create(Navigable::MENU_TOOLS, function (MenuBuilder $tools) {
            ...
            $tools->route('clear-cache', 'Clear Cache', [], 10, ['icon' => 'fa fa-refresh']);
